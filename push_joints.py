...
def use_pose_attr(push_jnt, input_jnt, name, region, axis, start_val, end_val, rmp_pos_val):
    """
    if use pose attribute, create pose attr and connect to pose
    """
    pass

def add_pose_to_push(push_jnt, input_jnt, name, region, axis, start_val, end_val, rmp_pos_val):
    if not cmds.objExists(push_jnt):
        cmds.warning("Push joint does not exist.")
        return
    if not cmds.objExists(input_jnt):
        cmds.warning("Input joint (driver) does not exist.")
        return
    
    side = _side_from_name(push_jnt)
    if side not in ('l', 'r', 'c'):
        cmds.warning("Cannot parse side from push joint.")
        return
    
    push_idx = _push_index_from_name(push_jnt)
    offset_grp = push_jnt.replace('jnt_', 'offset_')
    if not cmds.objExists(offset_grp):
        cmds.warning(f"Offset group not found for {push_jnt}")
        return
    ...
    loc_name = f"loc_{side}_{region}_{name}_pushPose_{push_idx}_{local_i:04d}"
    loc = cmds.spaceLocator(name=loc_name)[0]
    cmds.matchTransform(loc, offset_grp)
    cmds.parent(loc, offset_grp)
    
    rmp_name = f'rmp_{side}_{region}_{name}_pushPose_{push_idx}_{local_i:04d}'
    ...
    if local_i > 1:
        prev_rmp = f'rmp_{side}_{region}_{name}_pushPose_{push_idx}_{(local_i - 1):04d}'
    ...
    mdT = cmds.createNode('multiplyDivide', n=f'mult_{side}_{region}_{name}_pushPose_trans_{push_idx}_{local_i:04d}')
    mdR = cmds.createNode('multiplyDivide', n=f'mult_{side}_{region}_{name}_pushPose_rot_{push_idx}_{local_i:04d}')
    mdS = cmds.createNode('multiplyDivide', n=f'mult_{side}_{region}_{name}_pushPose_scale_{push_idx}_{local_i:04d}')
    ...
    for attr, md in (('translate', mdT), ('rotate', mdR)):
        ...
        if not cmds.objExists(pma_name):
            ...
            pma_node = pma_name
        connect_attr(md, 'output', pma_node, f'input3D[{local_i - 1}]', True)
    ...
    if local_i <= 2:
        ...
        connect_attr(mdS, "output", out, "input1" if local_i == 1 else "input2", True)
    else:
        prev_idx = local_i - 2
        this_idx = local_i - 1
        prev = f"{scale_base}_{prev_idx:04d}"
        out = f"{scale_base}_{this_idx:04d}"
        
        if not cmds.objExists(prev):
            prev = cmds.createNode("multiplyDivide", n=prev)
            if local_i == 3 and not cmds.listConnections(f"{push_jnt}.scale", s=True, d=False):
...
def auto_add_pose(push_jnt, input_jnt, name, region, axis, start_val, end_val, rmp1, rmp2):
    total = end_val - start_val
    one_third = start_val + total * (1.0 / 3.0)
    two_third = start_val + total * (2.0 / 3.0)
    
    # pose01 ex.(0-60)
    add_pose_both_sides(push_jnt, input_jnt, name, region, axis, start_val, two_third, rmp1)
    
    # pose02 ex.(30-90)
    add_pose_both_sides(push_jnt, input_jnt, name, region, axis, one_third, end_val, rmp2)
    
    # end ex.(60-90)
    add_pose_both_sides(push_jnt, input_jnt, name, region, axis, two_third, end_val, 1.0)
    
    # restore mid rmp (counter overwrite)
    side = _side_from_name(push_jnt)
    push_idx = _push_index_from_name(push_jnt)
...
def push_pose_ui():
    ...
    cmds.window("pushJointUI", title="Push Joint Builder", widthHeight=(440, 560))
    ...
    def on_auto_pose(*_):
        auto_add_pose(
            ...
            cmds.floatFieldGrp(start_val_field, q=True, value1=True),
            cmds.floatFieldGrp(end_val_field, q=True, value1=True),
            cmds.floatFieldGrp(auto_rmp1_field, q=True, value1=True),  # user first rmp
            cmds.floatFieldGrp(auto_rmp2_field, q=True, value1=True),  # user second rmp
        )
